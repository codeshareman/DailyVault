...
`Daily/YYYY/YYYYMMDD.md` 是唯一的日常记录源，按年分子目录（如 `Daily/2026/20260817.md`）。每个文件使用 `Templates/daily.md` 创建，并保持以下固定章节：今日计划、随手记录、输入、输出、生活时间线、学到、复盘、明日 / 迁移。

规则：

- 每天一个文件，放在当年的年度子目录（`Daily/{年份}/`）里。
- 今日计划最多三项；完成状态以复选框为准。
...
- “输入”章节自带 Dataview 块：按 `Clippings/` 文件的 `created` 字段自动列出当天的剪藏；手动输入写在列表项里，两者合成本日输入。复盘模板按周期自动归集剪藏输入（`weekNumber` / `month` / `quarter` / `year`）。
