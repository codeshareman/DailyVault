...
>
> 文章提出一套从业务目标到模型能力的分层指标方法，并建议通过“人人一致、人机一致”对齐主观标准，将模糊指标拆成可判断的 Rubric，结合客观评测、人工评测和 AI 评测。实践上应从高频核心场景和少量指标起步，持续收集 Good Case 与 Bad Case，经过采集、清洗、评测、质检、归因形成数据飞轮，而不是一开始设计复杂但难以执行的指标体系。
>
> 随着 Agent 从短程问答走向需要多步规划、工具调用和动态调整的长程任务，评测重点转向“事情是否做成，以及是怎么做成的”。面向 Task 的测试需要结合 Prompt、Expected Behavior 和 Trace，并逐步由人工主导转向人工定义标准、AI 承担规模化评测、平台负责回放与归因。最终，Agent 评测应成为具备全链路回放、Case 管理、执行沙箱、自动评分、回归验证和发布门禁能力的基础设施。
