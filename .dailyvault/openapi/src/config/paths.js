...
const OPENAPI_ROOT = path.resolve(path.dirname(CURRENT_FILE), '..', '..');
const DEFAULT_VAULT_ROOT = path.resolve(OPENAPI_ROOT, '..', '..');

export function getOpenApiRoot() {
  return path.resolve(process.env.DAILYVAULT_OPENAPI_ROOT || OPENAPI_ROOT);
}
...
export function resolveOpenApiPath(relativePath = '.') {
  const root = getOpenApiRoot();
  const target = path.resolve(root, relativePath);
  if (target !== root && !target.startsWith(`${root}${path.sep}`)) {
    throw new Error(`路径逃逸 OpenAPI 根目录：${relativePath}`);
  }
  return target;
}
