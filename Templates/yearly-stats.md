...
const sectionCount = (name) => dailyItems.filter(l => l.section?.subpath === name).length;
...
dv.table(
  ...
  [
    ...
    ["随手记录 / 生活时间线 / 学到 / 复盘", ["随手记录", "生活时间线", "学到", "复盘"].map(name => sectionCount(name)).join(" / ") + " 条"],
  ]
);
```
## 今年做了什么
...
<!-- 汇集每日「随手记录」章节的原始内容，并保留来源 Daily 链接，便于查看上下文。 -->
...
const rows = lists
  ...
  .map(l => [cell(l), l.page.file.link]);
dv.table(["记录", "来源"], rows.length ? rows : [["（今年还没有随手记录）", ""]]);
```
