...
年度统计按聚合而非明细设计：KPI 概览（记录天数、任务完成率、今日计划达标率、最长连续记录、输入/输出条目）看全年结果；「今年做了什么 / 今年没做什么」给出可跳转的成果与未完成清单；月度活跃趋势用 dataviewjs 渲染 CSS 图表看节奏；年度计划完成率（年度计划文件「本期计划」）与月度计划完成率（每日今日计划）看计划执行；知识转化漏斗看「输入→学到→输出」三段转化率（是否只进不出）；`#kind/...` 条目分类带分布条形图（tag 可点击搜索 `tag:kind/xxx`）；输入与输出完整清单从每日「输入」「输出」章节派生，条目可点击跳转来源 daily（剪藏是输入的子集，随条目呈现）。任务统计按 status 精确区分（[x]/[ ]/[-]）。统计查询限定在年份目录（`FROM "{{date:YYYY}}"`），不逐日罗列。
