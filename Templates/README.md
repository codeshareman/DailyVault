...
Daily 模板保持低摩擦：最多三项计划，其他章节只保留一个自由记录入口。模板在每个章节标题下内置一行 HTML 注释标记，说明该章节的用途和适用的 `#kind/...` 标签——编辑时可见、阅读时隐藏、不进入 Dataview 统计。周期模板只保留本期总结、下期三项重点和 Dataview 汇总；不要加入 Source、Note 或领域专用字段。
