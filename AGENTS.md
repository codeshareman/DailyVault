...
周/月/季/年复盘以 `Daily/` 为唯一证据源。Dataview 可以按 Daily frontmatter 的 `week`、`month`、`quarter`、`year` 汇总页面、任务和固定章节下的列表项：章节条目统计、每日活跃度、计划完成情况（今日计划 checkbox）、未完成任务、`#kind/...` 条目分类，以及“随手记录”（闪念）、“输入”、“输出”、“学到”、“复盘”、“生活时间线”的内容归集。剪藏输入自动按 `Clippings/` 文件的 `created` 字段归集到 Daily 与复盘（当日剪藏列表 / 周期剪藏输入）。不要从正文推断未记录的状态或指标。
