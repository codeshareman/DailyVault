...
3. 文章、工具、课程和反馈写在“输入”（剪藏也在其中）；实际结果写在“输出”——可以是一篇文章、一个地址（URL）或一个交付物；生活经历写在“生活时间线”。
...
- **年度概览**：记录天数、总条目数、任务统计（完成/待办/取消/其他）、任务完成率、今日计划达标率、最长连续记录、输入/输出条目数；
- **今年做了什么 / 今年没做了什么**：可跳转的成果与未完成清单（完成的任务、输出、学到；真正的待办与取消项）；
- **月度活跃趋势与月度计划完成率**：dataviewjs 渲染的 CSS 图表，月份标签可点击搜索该月记录；年度计划完成率统计年度计划文件（`YYYY/Yearly/YYYY.md`）「本期计划」的完成情况；
- **知识转化漏斗**：输入 → 学到 → 输出 三段转化率（看是否只进不出）；
- **`#kind/...` 条目分类**：带分布条形图，tag 可点击搜索 `tag:kind/xxx`；
