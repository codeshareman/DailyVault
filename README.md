...
- 早上：只做启动取舍：写 `今天最值得变好的一件事`，再勾出 `今天最难但最重要的一步`。另外两件重要事可以留空。
- 白天：所有事实先丢进 `随手记录`，不急着分类；QuickAdd 的“追加到今天”也写到这里。
- 遇到链接：先贴在 Daily；如果值得保留，对 AI 说“把这个链接整理成 Source”。如果是 Web Clipper 已抓取的内容，告诉 AI 具体 clipping 文件名或链接。
- 有想法但没成型：可以先放 `随手记录`；如果明显不是今天要处理，再放 `Notes/` 或 `Inbox.md`。
- 晚上：对 AI 说“帮我做 daily closeout / 今日收尾，不要编没有证据的内容”，让 AI 把 `随手记录` 分拣成做了什么、输入、输出、生活时间线、学习/复盘，再补 `明天第一步 / 迁移`。
...
- 先用自然语言写 Daily；空白可以保留，今天只写一句也算记录。
- 默认页面要短，打开后能立即写；早上只需要写一句主线，并勾出一个最难但最重要的行动。
- 使用今日启动、另外两件重要事、随手记录、输入、输出、做了什么、生活时间线、学习/复盘和明天第一步 / 迁移作为每日骨架。
...
- 每日收尾：总结今天的输入、输出、执行、生活时间线、学习、阻碍、明天第一步和需要迁移 / 放弃的事项。
